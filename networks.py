...
class ganvo(nn.Module):
	...
	def optimize_G(self):
		...
		loss_adv_g = self.ganloss(self.discriminator(const_from_t_2),True)
	...
	def optimize_D(self):
		...
		fake_image = self.fake_pool.query(self.const_from_t_1)
		loss_fake = self.ganloss(self.discriminator(fake_image.detach()),False)
